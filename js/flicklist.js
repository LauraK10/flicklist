...
function searchMovies(query, callback) {
  ...
  $.ajax({
    ...
    success: function(response) {
      ...
      {
        keywordIDs.push(response.results[obj].id);
      }
      
      
      /*response.results.map(function(num){
        for (var obj in response.results)
        {
          
          console.log(response.results[obj].id)
          keywordIDs.push(response.results[obj].id);
          //return response.results[obj].id;
        }
    
      });*/
      var keyIDs = response.results.map(function(elem){
        return elem.id;
      });
      
      var keywordsString = keyIDs.join('|');
